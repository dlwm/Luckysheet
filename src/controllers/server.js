...
const server = {
	...
	afterConnection: null,//连接成功后执行
	wsError:null,//ws连接异常执行
	...
	openWebSocket: function(){
		...
		if('WebSocket' in window){
			...
			_this.websocket.onclose = function(e){
				console.info(locale().websocket.close,e.code);
				clearInterval(_this.retryTimer)
				_this.retryTimer = null
				if(e.code !== 1000){
					if(_this.wsError !== null){
						_this.wsError()
					}
				} else {
	...
	closeWebSocket: function () {
		let _this = this;
		if($$('#luckysheet-icon-morebtn-div').style.visibility = 'visible'){
			$$('#luckysheet-icon-morebtn-div').style.visibility = 'hidden';
		}
		if ("WebSocket" in window) {
			console.log(this);
			_this.websocket.close(1000,'手动关闭');
